...
> ## 🚧 Work in progress — Phase 3 (outside-in diligence) complete, no frontend built
>
...
| **Phase 2.5 — Freshness audit + triage calibration** | ✅ Complete |
| **Phase 3 — Outside-in diligence: ARMADA Marine Robotics** | ✅ Complete, awaiting review |
...
**Phase 3 produced:** an outside-in diligence case on ARMADA Marine Robotics — source reconciliation that resolved the EPADS IP question (ARMADA is a **joint applicant with WHOI** on WO 2024/136933, not merely a licensee) and reconciled the Navy contract to base + exercised option + incremental CLIN funding; technical, commercial, competitive and business-model diligence by product line; a bottom-up procurement market built from 87 audited federal contracts; a scenario model in Excel with live formulas; a primary-research call plan; a 45-claim evidence register; and a **HOLD — need more evidence** recommendation. No INVEST recommendation is made on public information.
...
│   ├── armada_investment_memo.md    Phase 3 outside-in diligence memo
│   └── armada/                      Reconciliation, technical, commercial,
│                                    procurement, competitive, business model,
│                                    debates, call plan, evidence register
├── models/armada_underwriting.xlsx  Scenario model (live formulas)
├── src/ofr/
...
python3 src/ofr/models/procurement_audit.py     # bottom-up market audit
python3 src/ofr/models/armada_underwriting.py   # rebuild the scenario workbook
python3 -m pytest tests -q             # 154 tests, no network required
```
